...
export function timingToHours(value) {
  ...
  if (s.includes('мин')) return num / 60;
  if (s.includes('час')) return num;
  if (s.includes('день') || s.includes('дня') || s.includes('дней')) return num * 24;
...
export function normalizeTelegramPrefillUrl(url) {
  ...
  try {
    ...
    if (u.hostname.includes('t.me') && u.searchParams.has('text')) {
