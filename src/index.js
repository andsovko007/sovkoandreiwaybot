    ...
    if (!user.followupsSent?.day3 && elapsedHours >= parseDayHours(content, 3)) {
      if (!shouldSkipDay3(user)) {
        if (user.engaged) {
          ...
          if (voice?.text) {
            await bot.api.sendMessage(user.userId, toTelegramHtml(voice.text), { parse_mode: 'HTML', disable_web_page_preview: true });
          }
          if (row?.text) {
            ...
            await bot.api.sendMessage(user.userId, toTelegramHtml(text), { parse_mode: 'HTML', disable_web_page_preview: true, reply_markup: kb });
          }
        } else {
          const row = content.followups.find((r) => r.day === 3 && r.state === 'Молчун' && r.stopor === user.stoporLabel);
          if (row?.text) {
            await bot.api.sendMessage(user.userId, toTelegramHtml(row.text), { parse_mode: 'HTML', disable_web_page_preview: true });
          }
        }
      }
