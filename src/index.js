...
function getVoiceLink(content) {
  return normalizeTelegramPrefillUrl(content.settings['Ссылка: голосовое'] || '');
}

function getCallLink(content) {
  return normalizeTelegramPrefillUrl(content.settings['Ссылка: разбор'] || '');
}
...
function resultActionKeyboard(buttons) {
  return appendResultActionRows(new InlineKeyboard(), buttons);
}

function actionLinkKeyboard(text, url, actionButtons, options = {}) {
  if (!hasText(url)) return actionButtons ? resultActionKeyboard(actionButtons) : undefined;
  const kb = new InlineKeyboard().url(text, url).row();
  return actionButtons ? appendResultActionRows(kb, actionButtons, options) : kb;
}
...
function refineQuestionKeyboard(question, questionIndex, actionButtons) {
  const kb = new InlineKeyboard();
  question.options.forEach((opt, optIndex) => {
    kb.text(opt, `refine:${questionIndex}:${optIndex}`).row();
  });
  return appendResultActionRows(kb, actionButtons, { includeRefine: false });
}

async function sendStart(ctx) {
  const content = await getContent();
  const quizLink = getQuizLink(content);
  const extra = hasText(quizLink)
    ? { reply_markup: miniAppKeyboard(content.start.button || 'Пройти диагностику', quizLink) }
    : {};
...
async function askRefineQuestion(ctx, stoporLabel, questionIndex, content) {
  ...
  await sendText(ctx, question.text, {
    reply_markup: refineQuestionKeyboard(question, questionIndex, content.resultButtons),
  });
  return question;
}
...
function parseDayHours(content, dayNumber) {
  return timingToHours(content.settings[`День ${dayNumber}`]);
}
...
function shouldSkipDay3(user) {
  return Boolean(user.refineCompleted || user.voiceRequested || user.callRequested);
}

function shouldSkipDay5(user) {
  return Boolean(user.refineCompleted || user.voiceRequested || user.callRequested);
}
...
async function maybeSendVslAfterResult(userId, stoporLabel) {
  setTimeout(async () => {
    ...
    if (media?.text) {
      await bot.api.sendMessage(userId, toTelegramHtml(media.text), {
        parse_mode: 'HTML',
        disable_web_page_preview: true,
      });
    }
...
async function handleDay5Keyword(ctx, user, content) {
  ...
  await appendJourneyStep(user.userId, `День 5: ${normalized}`);
...
bot.callbackQuery(/^action:(voice|refine|call)$/, async (ctx) => {
  ...
  if (action === 'voice') {
    ...
    const kb = actionLinkKeyboard(buttonLabel, getVoiceLink(content), content.resultButtons);
  ...
  if (action === 'call') {
    ...
    const kb = actionLinkKeyboard(buttonLabel, getCallLink(content), content.resultButtons);
    await sendText(ctx, text, kb ? { reply_markup: kb } : {});
    const fresh = await getUser(user.userId);
...
bot.callbackQuery(/^day1:(money|overload)$/, async (ctx) => {
  ...
  const journeyStep = ctx.match[1] === 'money' ? 'День 1: Денег/клиентов не хватает' : 'День 1: Я уже не вывожу';
  await appendJourneyStep(user.userId, journeyStep);
...
bot.callbackQuery('want:call', async (ctx) => {
  ...
  await sendText(ctx, text, kb ? { reply_markup: kb } : {});
  if (user) {
    await appendJourneyStep(user.userId, 'Хочу разбор');
    await markEngaged(user.userId);
    const fresh = await getUser(user.userId);
    await sendAdminAlert('Написал хочу', fresh || user, content, { button: 'Хочу разбор' });
  }
});
  ...
  for (const user of users) {
    ...
    if (!user.followupsSent?.day1 && elapsedHours >= parseDayHours(content, 1)) {
      if (!shouldSkipDay1(user)) {
        const row = content.followups.find((r) => r.day === 1 && r.state === 'Не взаимод. + не записался' && r.stopor === 'Все');
        if (row?.text) {
          const kb = new InlineKeyboard()
            .text(row.button1 || 'Денег/клиентов не хватает', 'day1:money').row()
            .text(row.button2 || 'Я уже не вывожу', 'day1:overload').row();
          await bot.api.sendMessage(user.userId, toTelegramHtml(row.text), {
            parse_mode: 'HTML',
            disable_web_page_preview: true,
            reply_markup: kb,
          });
    ...
    if (!user.followupsSent?.day3 && elapsedHours >= parseDayHours(content, 3)) {
      if (!shouldSkipDay3(user)) {
        if (user.engaged) {
          const voice = content.media.find((m) => m.day === '3' && m.type === 'Войс 30-40с' && m.stopor === user.stoporLabel);
          const row = content.followups.find((r) => r.day === 3 && r.state === 'Взаимодействовал + не записался' && r.stopor === 'Все');
          if (voice?.text) {
            await bot.api.sendMessage(user.userId, toTelegramHtml(voice.text), {
              parse_mode: 'HTML',
              disable_web_page_preview: true,
            });
          }
          if (row?.text) {
            ...
            const kb = row.button1 ? new InlineKeyboard().text(row.button1, 'want:call').row() : undefined;
            await bot.api.sendMessage(user.userId, toTelegramHtml(text), {
              parse_mode: 'HTML',
              disable_web_page_preview: true,
              reply_markup: kb,
            });
          ...
          const row = content.followups.find((r) => r.day === 3 && r.state === 'Молчун' && r.stopor === user.stoporLabel);
          if (row?.text) {
            await bot.api.sendMessage(user.userId, toTelegramHtml(row.text), {
              parse_mode: 'HTML',
              disable_web_page_preview: true,
    ...
    if (!user.followupsSent?.day5 && elapsedHours >= parseDayHours(content, 5)) {
      if (!shouldSkipDay5(user) && !isBooked(user)) {
        const row = content.followups.find((r) => r.day === 5 && r.state === 'Не записался' && r.stopor === 'Все');
        if (row?.text) {
          await bot.api.sendMessage(user.userId, toTelegramHtml(row.text), {
            parse_mode: 'HTML',
            disable_web_page_preview: true,
    ...
    if (!user.followupsSent?.day7 && elapsedHours >= parseDayHours(content, 7)) {
      if (!isBooked(user)) {
        const row = content.followups.find((r) => r.day === 7 && r.state === 'Не записался' && r.stopor === 'Все');
        if (row?.text) {
          await bot.api.sendMessage(user.userId, toTelegramHtml(row.text), {
            parse_mode: 'HTML',
            disable_web_page_preview: true,
