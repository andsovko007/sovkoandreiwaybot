...
async function handleDay5Keyword(ctx, user, content) {
  ...
  if (row?.text) {
    const media = findMediaEntry(content, '5', 'Общий', 'vsl');
    const out = row.text.replace('[VSL общий]', '').trim();
    if (hasText(out)) await sendText(ctx, out);
    await sendMediaEntry(ctx.chat.id, media);
  }
  ...
  for (const user of users) {
    ...
    if (!user.followupsSent?.day3 && elapsedHours >= parseDayHours(content, 3)) {
      if (!shouldSkipDay3(user)) {
        if (user.engaged) {
          const voice = findMediaEntry(content, '3', user.stoporLabel, 'войс');
          const row = content.followups.find((r) => r.day === 3 && r.state === 'Взаимодействовал + не записался' && r.stopor === 'Все');
          await sendMediaEntry(user.userId, voice);
          if (row?.text) {
